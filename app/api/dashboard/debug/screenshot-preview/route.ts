...
type ParserDiagnostic =
  ...
  // Stage BA-2D, Step 5 — "market_mismatch" added the same way, mirroring
  // lib/ai/betParser.ts's own ParseBetSlipResult widening. This route only
  // ever displays `code` as an operator-facing diagnostic field (never
  // branches on it), so the new value needs no other change here.
  | { mode: "OCR"; durationMs: number; valid: false; code?: "timeout" | "numeric_mismatch" | "market_mismatch"; error: string };
