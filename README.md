...
- **Rate limiting on `POST /api/miniapp/bets/text/preview`, `/text/confirm`, and `/screenshot/preview` is best-effort only.** Each route is gated by its own in-memory, per-verified-Telegram-user request counter (`lib/rateLimit/requestRateLimiter.ts`), keyed after Telegram auth succeeds and before the first AI/OCR/odds-provider call. Current **provisional MVP defaults** (constants live in each route module, not here, so they don't go stale): text preview 5 requests/60s, screenshot preview 3 requests/60s, text confirm 10 requests/60s (SINGLE and EXPRESS confirmations share one bucket per player). It is **in-memory and per serverless instance, not a distributed quota**: it resets on cold start and is not shared across concurrent Vercel instances or regions, so an attacker whose requests land on enough different warm instances can still exceed the intended limit. `maxTrackedKeys` is a hard per-instance cap on how many distinct users' quota state a given warm instance tracks at once; if that cap is reached with every tracked bucket still active (nothing yet expired), a brand-new, previously-unseen user is allowed through *untracked* rather than either evicting an existing user's live quota or rejecting the new user outright — an intentional best-effort fail-open admission policy, not a silent bypass of the whole mechanism. It meaningfully raises the cost of casual abuse from a single warm instance; it does not provide a production-grade, globally-enforced rate limit. Strict, cross-instance enforcement would require a durable external store (e.g. Redis/Upstash) and remains future infrastructure work, deliberately not introduced yet — see `lib/rateLimit/requestRateLimiter.ts`'s file header for the full reasoning (the same trade-off already accepted for `lib/auth/loginRateLimit.ts`).
